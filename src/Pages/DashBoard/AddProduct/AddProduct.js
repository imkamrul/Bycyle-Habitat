...
const AddProduct = () => {
    ...
    return (
        <Container className="py-5">
            ...
            <Modal show={showProductModal} onHide={handleProductModelClose} backdrop="static" keyboard={false} >
                <Modal.Header closeButton>
                    <Modal.Title>Hello {user?.displayName}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    Do you want to add <span className="fw-bold">{newProduct?.name}</span> in products  ?
                </Modal.Body>
                <Modal.Footer>
